(function () {
  ...
  const GAME_VELOCITY = 50;

  const GAME_MAX_VELCOCITY = 200;

  const CLOUD_VELOCITY = 800;
  ...
  class Controller {
    ...
    checkCollision(player, obstacle) {
      ...
      if (
        ...
        player.coordX + player.spirtWidth * this.collisionFactor >
          obstacle.coordX
      ) {
        XCollision = true;
      }
    ...
    init = () => {
      ...
      if (
        ...
        this.height = (document.documentElement.clientWidth - 50) * this.ratio;
      } else {
        this.height = (document.documentElement.clientHeight - 50) * this.ratio;
      }

      this.context.imageSmoothingEnabled = false;
      this.context.canvas.setAttribute("height", this.height);
      this.context.canvas.setAttribute("width", this.width);
      this.counterSpirtSize = this.counterSpirtFactor * this.height;
      this.gameLoseSpirtWidth = this.gameLoseSpirtFactor * this.heght;
      this.gameLoseSpirtHeight = this.gameLoseSpirtWidth / 2.6;
      this.playerWidth = this.playerWidthFactor * this.height;
      this.obstacleWidth = this.obstacleWidthFactor * this.height;
      this.groundWidth = this.groundWidthFactor * this.height;
      this.cloudWidth = this.cloudWidthFactor * this.height;
      this.playerMaxJump = Ground.groundHeight + this.obstacleWidth * 5;
      ...
      this.player = new Player(
        ...
        this.height,
        50,
        Ground.groundHeight - this.playerCoordYOffset * this.obstacleWidth,
        this.playerMaxJump
      );
    ...
    update = (deltaTime) => {
      if (this.gameVelocity < GAME_MAX_VELCOCITY) {
        this.gameVelocity = this.gameVelocity + GAME_ACC;
      }

      let _simulatedVelocity = this.gameVelocity * deltaTime;
      ...
      if (this.gameState == "PLAY") {
        ...
        this.obstacles.update(_simulatedVelocity, this.context);
      ...
      this.clouds.update(deltaTime, this.gameVelocity);
      this.ground.update(_simulatedVelocity);
      this.player.update(deltaTime, this.gameVelocity);
    };
    ...
    play = () => {
      ...
      this.player.start();
      this.gameState = "PLAY";
      this.gameVelocity = GAME_VELOCITY;
  ...
  class Player {
    constructor(playerWidth, height, coordX, coordY, maxJump) {
      ...
      this.minJumpHeight = height - maxJump;
    ...
    update(deltaTime, velocity) {
      ...
      if (this.jumping && this.alive) {
        let jumpVelocity = 1.5 * velocity * deltaTime;

        if (this.coordY <= this.minJumpHeight) {
          this.coordY = this.minJumpHeight;
          this.falling = true;
        }

        if (this.falling) {
          this.coordY += jumpVelocity;
          if (this.coordY >= this.initialCoordY) {
            console.log("RUN");
  ...
  class Obstacles {
    ...
    init = (obstacleWidth, coordY, canvasWidth, canvasHeight) => {
      ...
      this.minDistBtwObstacles = this.spirtWidth * 7;
      ...
      this.obstacles = this.generateObstacles(
        Math.ceil(canvasWidth / this.minDistBtwObstacles)
      );
    };
    ...
    reset() {
      this.obstacles = this.generateObstacles(
        Math.ceil(this.canvasWidth / this.minDistBtwObstacles)
      );
    }
    ...
    generateObstacles = (
      ...
      let obstacles = [];
      console.log("Number of Obstacles", numberOfObstacles);
      for (let i = 0; i < numberOfObstacles; i++) {
        let randomInt = Math.floor(Math.random() * this.obstacleSpirts.length);
        let obstacle = {
          ...this.obstacleSpirts[randomInt],
        };
        obstacle.coordX = canvasWidth / 2 + i * this.minDistBtwObstacles;
        obstacle.coordY = this.coordY - obstacle.spirtHeight;
  ...
  class Clouds {
    constructor(cloudWidth, canvasWidth, cloudSpawnArea) {
      ...
      this.spirtsIndex = 0;
      this.cloudSpeed = CLOUD_VELOCITY;
    ...
    update(deltaTime, gameVelocity) {
      let newCloudAdd = false;
      this.clouds = this.clouds.map((cloud) => {
        let cloudSpeed = (this.cloudSpeed - gameVelocity) * deltaTime;
        ...
        if (cloudSpeed < 0 && cloud.movingForward) {
          cloud.coordX =
            -this.spirtWidth - Math.random() * 1.25 * this.canvasWidth;
    ...
    generateCloud = (
      ...
      if (numberOfClouds === 1 && this.cloudSpirts.length) {
        ...
        cloud.coordX = canvasWidth + Math.random() * canvasWidth;
      ...
      for (let i = 0; i < numberOfClouds; i++) {
        ...
        let cloud = {
          ...
          coordX: Math.random() * 2 * canvasWidth,
  ...
  window.addEventListener("load", function (event) {
    ...
    let accumalator = 0;
    const steps = 6 / FPS;
    function FrameReqCallback() {
      let now = getTimeStamp();
      let deltaTime = now - (lastTime || now);
      lastTime = now;
      accumalator += deltaTime / 1000;

      while (accumalator > steps) {
        accumalator -= steps;
        gameController.update(steps);
      }
      gameController.draw();
